...
static sgx_status_t SGX_CDECL sgx_ecall_nacl_main(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_nacl_main_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_nacl_main_t* ms = SGX_CAST(ms_ecall_nacl_main_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	char** _tmp_argv = ms->ms_argv;



	ms->ms_retval = ecall_nacl_main(ms->ms_argc, _tmp_argv);
...
SGX_EXTERNC const struct {
	...
	4,
	{
		...
		{(void*)(uintptr_t)sgx_ecall_nacl_main, 0},
	}
};

SGX_EXTERNC const struct {
	size_t nr_ocall;
	uint8_t entry_table[38][4];
} g_dyn_entry_table = {
	38,
	{
		{0, 0, 0, 0, },
