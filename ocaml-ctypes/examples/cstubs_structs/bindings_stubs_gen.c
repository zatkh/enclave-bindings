...
int main(void)
{
  ...
  ctypes_printf("    s'.spec <- Complete { size = %zu; align = %zu }\n",
               sizeof(struct tm),
               offsetof(struct { char c; struct tm x; }, x));
  ...
  puts("let constant (type t) name (t : t typ) : t = match t, name with");
  {
     ...
     short v = (SHRT_MAX);
     ctypes_printf("  | Ctypes_static.Primitive Cstubs_internals.Short, \"SHRT_MAX\" ->\n    %hd\n",
                  v);
